...
//            #ifdef TCPIP_NETWORK_DEFAULT_IP_ADDRESS_2  //moi!!!
//                while(!_TCPIPStackIsConfig(pIf) && ((pArpPkt->TargetIPAddr.Val == pIf->netIPAddr.Val) || (pArpPkt->TargetIPAddr.Val == tcpip_address_2.Val)))
//            #else
                while(!_TCPIPStackIsConfig(pIf) && (pArpPkt->TargetIPAddr.Val == pIf->netIPAddr.Val))
//            #endif
...
//                    #ifdef TCPIP_NETWORK_DEFAULT_IP_ADDRESS_2  //moi!!!
//                        if(!_ARPSendIfPkt(pIf, ARP_OPERATION_RESP, (uint32_t)pArpPkt->TargetIPAddr.Val, (uint32_t)pArpPkt->SenderIPAddr.Val, dstMAC))
//                    #else
                        if(!_ARPSendIfPkt(pIf, ARP_OPERATION_RESP, (uint32_t)pIf->netIPAddr.Val, (uint32_t)pArpPkt->SenderIPAddr.Val, dstMAC))
//                    #endif
                    {
