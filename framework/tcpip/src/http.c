...
static bool TCPIP_HTTP_FileSend(HTTP_CONN* pHttpCon)
{
    ...
    switch(pHttpCon->file_sm)
    {
        ...
        case SM_SERVE_TEXT_DATA:
            ...
            #if defined(Table_Mix)
            extern char FichierCSV[20];
            extern char DossierCSV[30];
            sprintf(DossierCSV,"CSV_Config/%s",FichierCSV);
            if(!strcmp(pHttpCon->fileName,DossierCSV))
            #else
            if(!strcmp(pHttpCon->fileName,"Config.csv"))
            #endif
            {
                extern char ConfigCSV[1000];
                static unsigned int cursor = 0, size;
                
                #if defined(Table_Mix)
                extern int Config_Used;
//                char httpDataBuff[20];
                DownloadCSV(Config_Used);
//                strcpy(httpDataBuff, "/Save_Config.htm");
//                TCPIP_HTTP_CurrentConnectionStatusSet((HTTP_CONN_HANDLE)0xA0017F30, HTTP_REDIRECT);
//                TCPIP_HTTP_Print((HTTP_CONN_HANDLE)0xA0017F30,0x00000261);
                
                size = strlen(ConfigCSV);
                bytesPut = TCPIP_TCP_ArrayPut(pHttpCon->socket, ConfigCSV + cursor, size - cursor);
                #endif
                ...
                cursor += bytesPut;
                if(cursor < size) // Correspond à la taille max d'un fichier (supposition)
                {
                    pHttpCon->TxFile.bytesReadCount+=bytesPut;
                    //pHttpCon->TxFile.numBytes = cursor;
                }
                else
                {
