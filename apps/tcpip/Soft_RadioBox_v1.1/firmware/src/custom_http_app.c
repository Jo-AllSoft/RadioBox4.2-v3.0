    ...
    if(!memcmp(filename, "Input_Audio.htm", 15))
    {
        if(Config_Used != 0)
        {
            if(TCPIP_HTTP_ArgGet(httpDataBuff,(uint8_t*)"Redu") != 0)
            {
                ...
                Nconfig[Config_Used].Device_IN[In_Audio].ReductionMax = atof(ptr)*1.25;
            ...
            if(TCPIP_HTTP_ArgGet(httpDataBuff,(uint8_t*)"RefV") != 0)
            {
                ...
                memcpy(tmp, ptr, 20);
                Somm_Volume = atof(ptr);

                // <editor-fold defaultstate="collapsed" desc="Gestion Volume et Gain">            
                if((Somm_Volume > 0)&&(Somm_Volume <= 3.75))
                {
                    Nconfig[Config_Used].Device_IN[In_Audio].Gain = 3.75;            
                }
                else if((Somm_Volume > 3.75)&&(Somm_Volume <= 7.5))
                {
                    Nconfig[Config_Used].Device_IN[In_Audio].Gain = 7.5;               
                }
                else if((Somm_Volume > 7.5)&&(Somm_Volume <= 11.25))
                {
                    Nconfig[Config_Used].Device_IN[In_Audio].Gain = 11.25;               
                }
                else if(Somm_Volume <= 0)
                {
                    Nconfig[Config_Used].Device_IN[In_Audio].Gain = 0;
                }
                else
                {
                    return;
                }
                Perte = Nconfig[Config_Used].Device_IN[In_Audio].Gain - Somm_Volume;

                Nconfig[Config_Used].Device_IN[In_Audio].Volume = -Perte; 
                if(Nconfig[Config_Used].Device_IN[In_Audio].Volume < -30)
                {
                    Nconfig[Config_Used].Device_IN[In_Audio].Volume = -78.75;
                }            
                // </editor-fold>

                I2C_PT_AudioSwitch(Select_Input(In_Audio), Nconfig[Config_Used].Device_IN[In_Audio].Gain,Nconfig[Config_Used].Device_IN[In_Audio].Loudness,Stereo1);
...
void TCPIP_HTTP_Print_InSelect(HTTP_CONN_HANDLE connHandle,uint16_t ShapeNr)
{
    if(In_Audio == 3)
    {
