...
SNMP_TRAP_INFO trapInfo={TCPIP_SNMP_TRAP_TABLE_SIZE};
...
static bool SNMPSendNotification(uint8_t receiverIndex, SNMP_ID var, SNMP_VAL val,uint8_t targetIndex)
{
    ...
    // Convert local to network order.
    IPAddress.v4Add.v[0] = trapInfo.table[receiverIndex].IPAddress.v[0];
    IPAddress.v4Add.v[1] = trapInfo.table[receiverIndex].IPAddress.v[1];
    IPAddress.v4Add.v[2] = trapInfo.table[receiverIndex].IPAddress.v[2];
    IPAddress.v4Add.v[3] = trapInfo.table[receiverIndex].IPAddress.v[3];
    ...
    switch(smState)
    {
        case SM_PREPARE:
            ...
            tempRxIndex=receiverIndex;
            // Convert local to network order.
            tempIpAddress.v[0] = trapInfo.table[receiverIndex].IPAddress.v[0];
            tempIpAddress.v[1] = trapInfo.table[receiverIndex].IPAddress.v[1];
            tempIpAddress.v[2] = trapInfo.table[receiverIndex].IPAddress.v[2];
            tempIpAddress.v[3] = trapInfo.table[receiverIndex].IPAddress.v[3];
            // get the specific SNMP trap notification type
            TCPIP_SNMP_TrapSpecificNotificationGet(&specTrap);
...
//*/
//            TCPIP_SNMP_TrapSpecificNotificationSet(1,ENTERPRISE_SPECIFIC,SNMP_DEMO_TRAP_1);
...
/**************************************************************************
  ...
  Remarks:
    ...
    and send trap.
 ***** */
#define SNMP_TX_SIZE 10
int SNMP_active = 0, SNMP_active_Swox = 0, snmp_activation = 1;
SNMP_ID SNMP_trap[SNMP_TX_SIZE], SNMP_id[SNMP_TX_SIZE];
SNMP_VENDOR_SPECIFIC_TRAP_NOTIFICATION_TYPE SNMP_generic[SNMP_TX_SIZE];
SNMP_VENDOR_SPECIFIC_TRAP_NOTIFICATION_TYPE SNMP_generic_Swox[SNMP_TX_SIZE];
SNMP_ID SNMP_trap_Swox[SNMP_TX_SIZE], SNMP_id_Swox[SNMP_TX_SIZE];
void SNMPTrap(void)
{
    static uint32_t TimerRead=0;
//    static bool buttonPushNotify=false;
    static uint8_t trapReceiverNotfyCntr=0;
    static SNMP_VAL buttonPushval;
//    static uint8_t buttonLock=false;
    static uint8_t timeLock=false;
    uint8_t     targetIndex;
    bool        retVal=true;
    //static uint8_t  trapIndex=0;
    ...
    if(trapReceiverNotfyCntr==trapInfo.Size - 1)
    {
        // Fin de la transmission SNMP
        trapReceiverNotfyCntr = 0;
        SNMP_active --;
    }
    
    if(SNMP_active > 0)
    {
        if (trapInfo.table[trapReceiverNotfyCntr].Flags.bEnabled)
        {
             if(netStartForTrap)
            {
                // select and set the trap interface
    ...
    */
            ...
            if(TCPIP_SNMPV3_TrapTypeGet())
            {
                ...
                {
                    TCPIP_SNMP_TrapSpecificNotificationSet(BUTTON_PUSH_EVENT,ENTERPRISE_SPECIFIC,SNMP_DEMO_TRAP_1);
                    TCPIP_SNMP_TrapSendFlagSet(false);
                    retVal = SNMPSendNotification(trapReceiverNotfyCntr, PUSH_BUTTON, buttonPushval,targetIndex);
                    if((gSendTrapSMstate == 0x0) && (retVal == false)) // gSendTrapSMstate == SM_PREPARE
                    {
                        retVal = SNMPSendNotification(trapReceiverNotfyCntr, PUSH_BUTTON, buttonPushval,targetIndex);
                    }
...
#endif
            {
                buttonPushval.byte = 1;
                TCPIP_SNMP_TrapSpecificNotificationSet(SNMP_generic[SNMP_active],ENTERPRISE_SPECIFIC,SNMP_trap[SNMP_active]);
                    TCPIP_SNMP_TrapSendFlagSet(false);
                    retVal = SNMPSendNotification(trapReceiverNotfyCntr, SNMP_id[SNMP_active], buttonPushval,targetIndex);
                if((gSendTrapSMstate == 0x0) && (retVal == false)) // gSendTrapSMstate == SM_PREPARE
                {
                    retVal = SNMPSendNotification(trapReceiverNotfyCntr, SNMP_id[SNMP_active], buttonPushval,targetIndex);
                }
...
//        buttonPushNotify = false;
        timeLock=false;
        TCPIP_SNMP_TrapSpecificNotificationSet(VENDOR_TRAP_DEFAULT,ENTERPRISE_SPECIFIC,SNMP_DEMO_TRAP_1);
        gSendTrapSMstate = 0;
        return;
    }
}
void SNMPTrap_Swox(void)
{
    static uint32_t TimerRead=0;
//    static bool buttonPushNotify=false;
    static uint8_t trapReceiverNotfyCntr=4;
    static SNMP_VAL buttonPushval;
//    static uint8_t buttonLock=false;
    static uint8_t timeLock=false;
    uint8_t     targetIndex;
    ...
    if(trapReceiverNotfyCntr==trapInfo.Size)
    {
        // Fin de la transmission SNMP
        trapReceiverNotfyCntr = 4;
        SNMP_active_Swox --;
    }
    
    if(SNMP_active_Swox > 0)
    {
        if (trapInfo.table[4].Flags.bEnabled)
        {
             if(netStartForTrap)
            {
                // select and set the trap interface
    ...
    */
            ...
            {
                buttonPushval.byte = 1;
                TCPIP_SNMP_TrapSpecificNotificationSet(SNMP_generic_Swox[SNMP_active_Swox],ENTERPRISE_SPECIFIC,SNMP_trap_Swox[SNMP_active_Swox]);
                    TCPIP_SNMP_TrapSendFlagSet(false);
                    retVal = SNMPSendNotification(4, SNMP_id_Swox[SNMP_active_Swox], buttonPushval,targetIndex);
                if((gSendTrapSMstate == 0x0) && (retVal == false)) // gSendTrapSMstate == SM_PREPARE
                {
                    retVal = SNMPSendNotification(4, SNMP_id_Swox[SNMP_active_Swox], buttonPushval,targetIndex);
                }
...
void SNMP_Send_Trap(SNMP_Type SNMP_type, int act)
{
    if(act > 0)
    {
        SNMP_active ++;
        ...
        {
            case MUX_ON:
                SNMP_id[SNMP_active] = Control_MUX_ON;
                SNMP_trap[SNMP_active] = TRAP_MUX_ON;
                SNMP_generic[SNMP_active] = Gen_MUX_ON;
            break;
            case MUX_OFF:
                SNMP_id[SNMP_active] = Control_MUX_OFF;
                SNMP_trap[SNMP_active] = TRAP_MUX_OFF;
                SNMP_generic[SNMP_active] = Gen_MUX_OFF;
            break;
            case AUDIO_ON:
                SNMP_id[SNMP_active] = Control_AUDIO_ON;
                SNMP_trap[SNMP_active] = TRAP_AUDIO_ON;
                SNMP_generic[SNMP_active] = Gen_AUDIO_ON;
            break;
            case AUDIO_OFF:
                SNMP_id[SNMP_active] = Control_AUDIO_OFF;
                SNMP_trap[SNMP_active] = TRAP_AUDIO_OFF;
                SNMP_generic[SNMP_active] = Gen_AUDIO_OFF;
            break;
            case INPUT_ON:
                SNMP_id[SNMP_active] = Control_INPUT_ON;
                SNMP_trap[SNMP_active] = TRAP_INPUT_ON;
                SNMP_generic[SNMP_active] = Gen_INPUT_ON;
            break;
            case INPUT_OFF:
                SNMP_id[SNMP_active] = Control_INPUT_OFF;
                SNMP_trap[SNMP_active] = TRAP_INPUT_OFF;
                SNMP_generic[SNMP_active] = Gen_INPUT_OFF;
            break;
            case TEST:
                SNMP_id[SNMP_active] = Control_TEST;
                SNMP_trap[SNMP_active] = TRAP_TEST;
                SNMP_generic[SNMP_active] = Gen_TEST;
            break;
            case INPUT_LOW:
                SNMP_id[SNMP_active] = Control_INPUT_LOW;
                SNMP_trap[SNMP_active] = TRAP_INPUT_LOW;
                SNMP_generic[SNMP_active] = Gen_INPUT_LOW;
            break;
            default:
                SNMP_active --;
        }
...
void SNMP_Send_Trap_Swox(SNMP_Type SNMP_type, int act)
{
    if(act > 0)
    {
        SNMP_active_Swox ++;
        ...
        {
            case MUX_ON:
                SNMP_id_Swox[SNMP_active_Swox] = Control_MUX_ON;
                SNMP_trap_Swox[SNMP_active_Swox] = TRAP_MUX_ON;
                SNMP_generic_Swox[SNMP_active_Swox] = Gen_MUX_ON;
            break;
            case MUX_OFF:
                SNMP_id_Swox[SNMP_active_Swox] = Control_MUX_OFF;
                SNMP_trap_Swox[SNMP_active_Swox] = TRAP_MUX_OFF;
                SNMP_generic_Swox[SNMP_active_Swox] = Gen_MUX_OFF;
            break;
            case AUDIO_ON:
                SNMP_id_Swox[SNMP_active_Swox] = Control_AUDIO_ON;
                SNMP_trap_Swox[SNMP_active_Swox] = TRAP_AUDIO_ON;
                SNMP_generic_Swox[SNMP_active_Swox] = Gen_AUDIO_ON;
            break;
            case AUDIO_OFF:
                SNMP_id_Swox[SNMP_active_Swox] = Control_AUDIO_OFF;
                SNMP_trap_Swox[SNMP_active_Swox] = TRAP_AUDIO_OFF;
                SNMP_generic_Swox[SNMP_active_Swox] = Gen_AUDIO_OFF;
            break;
            case INPUT_ON:
                SNMP_id_Swox[SNMP_active_Swox] = Control_INPUT_ON;
                SNMP_trap_Swox[SNMP_active_Swox] = TRAP_INPUT_ON;
                SNMP_generic_Swox[SNMP_active_Swox] = Gen_INPUT_ON;
            break;
            case INPUT_OFF:
                SNMP_id_Swox[SNMP_active_Swox] = Control_INPUT_OFF;
                SNMP_trap_Swox[SNMP_active_Swox] = TRAP_INPUT_OFF;
                SNMP_generic_Swox[SNMP_active_Swox] = Gen_INPUT_OFF;
            break;
            case TEST:
                SNMP_id_Swox[SNMP_active_Swox] = Control_TEST;
                SNMP_trap_Swox[SNMP_active_Swox] = TRAP_TEST;
                SNMP_generic_Swox[SNMP_active_Swox] = Gen_TEST;
            break;
            case INPUT_LOW:
                SNMP_id_Swox[SNMP_active_Swox] = Control_INPUT_LOW;
                SNMP_trap_Swox[SNMP_active_Swox] = TRAP_INPUT_LOW;
                SNMP_generic_Swox[SNMP_active_Swox] = Gen_INPUT_LOW;
            break;
            default:
                SNMP_active_Swox --;
        }
...
This ID is only related to trap ID. and this implementaion is only for TRAPv2 specific.
*/
        TCPIP_SNMP_TrapSpecificNotificationSet(1,AUTH_FAILURE,SNMP_DEMO_TRAP_1);
        netStartForTrap = false;
    }
...
//    TCPIP_Helper_StringToIPAddress("192.168.1.38", &trapInfo.table[0].IPAddress);
//
//
//    if(trapReceiverNotfyCntr==trapInfo.Size)
//    {
...
//#endif
//            {
//                TCPIP_SNMP_TrapSpecificNotificationSet(BUTTON_PUSH_EVENT,ENTERPRISE_SPECIFIC,trap);
//                    TCPIP_SNMP_TrapSendFlagSet(false);
//                    retVal = SNMPSendNotification(trapReceiverNotfyCntr, id, buttonPushval,targetIndex);
//                if((gSendTrapSMstate == 0x0) && (retVal == false)) // gSendTrapSMstate == SM_PREPARE
//                {
//                    retVal = SNMPSendNotification(trapReceiverNotfyCntr, id, buttonPushval,targetIndex);
//                }
