...
//    extern Calibration_Memoire Valeur_Calibration[6];
//    extern Calibration_Memoire Valeur_Calibration2[6];
...
//    sprintf(Config_Usine,"Name:base;;;;;;;%s;;;;;;;%sInput;Line1;Line2;Micro1;Micro2;Micro3;Micro4;%sGain;6.25;0;11.25;11.25;11.25;11.25;%sBass;2;2;10;10;10;10;%sTreble;8;8;14;14;14;14;%sLoudness;0;0;0;0;0;0;%sReductionMax;10;10;%s;;;;;;;%sOutput;Studio;Master;Phone1;Phone2;Phone3;Phone4;PhoneDJ%sMax Gain;-78.75;-7.50;11.25;11.25;11.25;11.25;11.25%sLeft;0;0;0;0;0;0;0%sRight;0;0;0;0;0;0;0%sBass;2;0;4;4;4;4;2%sTreble;14;0;4;4;4;4;8%sLoudness;0;0;0;0;0;0;0%sMaster Gain;;0;;;;;%s;;;;;;;%s/FinCSV;;;;;;;%s",CRLF,CRLF,CRLF,CRLF,CRLF,CRLF,CRLF,CRLF,CRLF,CRLF,CRLF,CRLF,CRLF,CRLF,CRLF,CRLF,CRLF,CRLF,CRLF );
     


//    I2C_Start();
//    I2C_New(I2C_ID_2, 0xA0, 400000, 10);
...
//    Upload_Audio_CSV(Config_Usine);  
    ...
    for(i =0; i < 6;i++)
    {
        Somm_Volume = Nconfig[Config_Used].Device_IN[i].Volume;

        if((Somm_Volume > 0)&&(Somm_Volume <= 3.75))
        {
            Nconfig[Config_Used].Device_IN[i].Gain = 3.75;            
        }
        else if((Somm_Volume > 3.75)&&(Somm_Volume <= 7.5))
        {
            Nconfig[Config_Used].Device_IN[i].Gain = 7.5;               
        }
        else if((Somm_Volume > 7.5)&&(Somm_Volume <= 11.25))
        {
            Nconfig[Config_Used].Device_IN[i].Gain = 11.25;               
        }
        ...
        Perte = Nconfig[Config_Used].Device_IN[i].Gain - Somm_Volume;

        Nconfig[Config_Used].Device_IN[i].Volume = -Perte; 
        if(Nconfig[Config_Used].Device_IN[i].Volume < -30)
        {
            Nconfig[Config_Used].Device_IN[i].Volume = -78.75;
        }   
    }
...
//    NconfigDefaut[4] = Nconfig[4];
    ...
    {    
        ...
        if((Nconfig[j].Master_Gain != 0)||(Nconfig[j].Master_Gain != 1))
        {
            Nconfig[j].Master_Gain == 0;
        }
        ...
        for(i = 0; i < 6;i++)
        {
            ...
            if((Nconfig[j].Device_IN[i].Volume > 0)||(Nconfig[j].Device_IN[i].Volume < -78.75))
            {
                Nconfig[j].Device_IN[i].Volume = 0;
            }
            if((Nconfig[j].Device_OUT[i].Gain != 0)&&(Nconfig[j].Device_IN[i].Gain != 3.75)&&(Nconfig[j].Device_IN[i].Gain != 7.5)&&(Nconfig[j].Device_IN[i].Gain != 11.25))
            {
