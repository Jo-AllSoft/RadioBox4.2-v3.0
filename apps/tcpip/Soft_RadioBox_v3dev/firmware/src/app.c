...
void InitConfigSong(int numConfig);
// *****************************************************************************
// *****************************************************************************
...
bool Muting_PT[6];//bool car int fonction pas ??? bizarre

bool changeConfigSong = false;
// *****************************************************************************
/* Application Data
...
//            }
            ...
            Fct_BTN_Init(BTN_Config_Escape,0xFF0000,0x000000,0x000000,444,1,35,35,23,"X");
            Fct_BTN_Init(BTN_Config_1,0x00FF00,0xDEDEDE,0xFFFFFF,10,50,460,30,23,"Config 1");
            Fct_BTN_Init(BTN_Config_2,0x00FF00,0xDEDEDE,0xFFFFFF,10,85,460,30,23,"Config 2");
            Fct_BTN_Init(BTN_Config_3,0x00FF00,0xDEDEDE,0xFFFFFF,10,120,460,30,23,"Config 3");
            Fct_BTN_Init(BTN_Config_4,0x00FF00,0xDEDEDE,0xFFFFFF,10,155,460,30,23,"Config 4");
            Fct_BTN_Init(BTN_Config_5,0x00FF00,0xDEDEDE,0xFFFFFF,10,190,460,30,23,"Config 5");
            Fct_BTN_Init(BTN_Config_Save,0x0000FF,0x00FFFF,0xFFFFFF,145,230,200,30,23,"Save");
...
//            static bool Muting_PT[6];
            ...
            static bool configSong = false;
            static uint8 Compare_Config_Used;

            if(configSong == false)
            {
                if(changeConfigSong == true)
                {
                    InitConfigSong(Config_Used);
                    changeConfigSong = false;
                }
                Compare_Config_Used = Config_Used;
                cmd(BITMAP_HANDLE(0));
    ...
    //                        Muting_PT[i-2] = 0;
                ...
                // </editor-fold>
                const int BTN_Acce_Config = 200;
                cmd(TAG(BTN_Acce_Config));
                cmd_text(240,8,21,OPT_CENTERX,Nconfig[Config_Used].Name_Config);
                static int cpt_Time_Touch;
                if(BTN_Acce_Config == rd8(REG_TOUCH_TAG))
                {            
                    cpt_Time_Touch++;
                }
                else if(cpt_Time_Touch >= 60)
                { 
                    configSong = true;
                    cpt_Time_Touch = 0;                  
                }
...
//                cmd(COLOR_RGB(255,255,255));
                if(true == Affichage_BTN(BTN_Config_Escape,ONE_CLICK)){configSong = false; Config_Used = Compare_Config_Used;}
                
                int Read_BTN_Config;
                for(Read_BTN_Config=BTN_Config_1; Read_BTN_Config<=BTN_Config_5;Read_BTN_Config++)
                {
                    Change_Text_BTN(Read_BTN_Config,Nconfig[Read_BTN_Config-12].Name_Config);
                    
                    if(true == Affichage_BTN(Read_BTN_Config,ONE_CLICK))
                    {
                        Config_Used = Read_BTN_Config-12;
                    }
                    if(Config_Used+12 == Read_BTN_Config){Affichage_BTN(Read_BTN_Config,MODE_ON);}else{Affichage_BTN(Read_BTN_Config,MODE_OFF);}
                }

                
                if(true == Affichage_BTN(BTN_Config_Save,ONE_CLICK))
                {
                    if(Config_Used != Compare_Config_Used)
                    {                      
                        EEPROM_SaveData(&Config_Used);
                        changeConfigSong = true;
                    }
                    configSong = false;
                }      

                cmd(COLOR_RGB(0,255,0));
                cmd_text(240,8,24,OPT_CENTERX,"Select config");
...
void InitConfigSong(int numConfig)
{
    ...
    for(i=0; i < 6;i++)
    {
        I2C_PT_Volume(Select_Input(i),  Nconfig[numConfig].Device_IN[i].Volume);
        I2C_PT_Speak_Att(Select_Input(i),Nconfig[numConfig].Device_IN[i].L,LEFT );
        I2C_PT_Speak_Att(Select_Input(i), Nconfig[numConfig].Device_IN[i].R,RIGHT);
        I2C_PT_AudioSwitch(Select_Input(i), Nconfig[numConfig].Device_IN[i].Gain,0,Stereo1);
        I2C_PT_BASS_TREBLE(Select_Input(i), Nconfig[numConfig].Device_IN[i].Basse,PT_BASS);
        I2C_PT_BASS_TREBLE(Select_Input(i), Nconfig[numConfig].Device_IN[i].Aigue,PT_TREBLE);
    }

    for(i=0; i < 7;i++)
    {
        I2C_PT_Volume(Select_Output(i), Nconfig[numConfig].Device_OUT[i].Volume);
        I2C_PT_Speak_Att(Select_Output(i),Nconfig[numConfig].Device_OUT[i].L,LEFT );
        I2C_PT_Speak_Att(Select_Output(i), Nconfig[numConfig].Device_OUT[i].R,RIGHT);
        I2C_PT_AudioSwitch(Select_Output(i), Nconfig[numConfig].Device_OUT[i].Gain,0,Stereo1);
        I2C_PT_BASS_TREBLE(Select_Output(i), Nconfig[numConfig].Device_OUT[i].Basse,PT_BASS);
        I2C_PT_BASS_TREBLE(Select_Output(i), Nconfig[numConfig].Device_OUT[i].Aigue,PT_TREBLE);
    }
}
