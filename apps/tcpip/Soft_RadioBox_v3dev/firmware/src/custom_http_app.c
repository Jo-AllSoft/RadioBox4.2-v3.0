...
void TCPIP_HTTP_Print_Config_Def(HTTP_CONN_HANDLE connHandle,uint16_t ShapeNr)
{    
    TCP_SOCKET sktHTTP;
    sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
...
//    TCPIP_TCP_StringPut(TCPIP_HTTP_CurrentConnectionSocketGet(connHandle),chaine);
//}
#include "../framework/tcpip/src/http_private.h"
char footer[700];
const char *ctrl_footer = footer;  
void TCPIP_HTTP_Print_footer(HTTP_CONN_HANDLE connHandle)
{

    extern char unique_name[50];

    HTTP_CONN* ptl=connHandle;

        sprintf(footer,"<div class=\"spacer\">&nbsp;</div><left> %s <div id=\"footer\">Swoxtelecom Copyright &copy </div></div></div></div></div></div></body></html>",VERSION_PROG);

    
    if(ptl->TxFile.EndOfCallBackFileFlag==0)        
        ctrl_footer = TCPIP_TCP_StringPut(TCPIP_HTTP_CurrentConnectionSocketGet(connHandle),ctrl_footer);
    else
        ctrl_footer = TCPIP_TCP_StringPut(TCPIP_HTTP_CurrentConnectionSocketGet(connHandle),footer);
    
    if(ctrl_footer - footer < strlen(footer))
    {
        ptl->TxFile.EndOfCallBackFileFlag=0;
    }
    else
    {
        ptl->TxFile.EndOfCallBackFileFlag=1;
    }
}
