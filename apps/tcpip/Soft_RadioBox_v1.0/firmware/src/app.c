        ...
        // <editor-fold defaultstate="collapsed" desc="Initialisation"> 
         case APP_STATE2_INIT:
            ...
            for(i=0; i < 6;i++)
            {
                ...
                I2C_PT_AudioSwitch(Select_Input(i),  Nconfig[Config_Used].Device_IN[i].Gain,Nconfig[Config_Used].Device_IN[i].Loudness,Stereo1);
            ...
            for(i=0; i < 7;i++)
            {
                ...
                I2C_PT_AudioSwitch(Select_Output(i), Nconfig[Config_Used].Device_OUT[i].Gain,Nconfig[Config_Used].Device_OUT[i].Loudness,Stereo1);
            ...
            for(i=1; i<= 7;i++)
            {
                Mem_BTN[i] = 0;//met les valeur a 0 des bouton pour eviter de presser 2 fois a la premiere utilisation
            }
...
//            for(i=0; i< 13;i++)
//            {
//                extern I2C_PT2314 I2C_Msgs_PT2314[I2C_NB_PT2314];
//                StateAck.ACK_PT2314[i] = I2C_Msgs_PT2314[i].isConfigurate;
//            }
...
//            extern int Gain_Master;
 ...
 //Deplscer           
//            static int Overload_Ligne[2];
//   
////            Overload_Ligne[0] = I2C_Read_Buffer(AD_PT_OUT,1);
////            Overload_Ligne[1] = I2C_Read_Buffer(AD_PT_OUT,3);          
//            Overload_Ligne[0] = APP_MoyGli(10,Overload_Ligne[0],I2C_Read_Buffer(AD_PT_OUT,1));
//            Overload_Ligne[1] = APP_MoyGli(10,Overload_Ligne[1],I2C_Read_Buffer(AD_PT_OUT,3));
            ...
            {  
                ...
                if(Mem_BTN[1] == 0)
                {
                    ...
                    if((Mem_BTN[2] == 0 || Valeur_Fader[0] < Seuil_Fader)&&(Mem_BTN[3] == 0|| Valeur_Fader[1] < Seuil_Fader)&&(Mem_BTN[4] == 0|| Valeur_Fader[2] < Seuil_Fader)&&(Mem_BTN[5] == 0|| Valeur_Fader[3] < Seuil_Fader))
                    {
                        ...
                        if( Muting_PT[i] == 1)
                        {
                            I2C_PT_Volume(Select_Input(i+2),PT_Muting);
                            Muting_PT[i] = 0;                       
                        }
                    ...
                    {                            
                        Change_Text_BTN(1,"ON-AIR SELECT");                            
            ...
            cmd(BITMAP_LAYOUT(ARGB1555, 294, 42));
            cmd(BITMAP_SIZE(NEAREST, BORDER, BORDER, 147, 42));
            
                    
            cmd(BITMAP_HANDLE(1));
            cmd(BITMAP_SOURCE(RAM_G+20280+12348));
            cmd(BITMAP_LAYOUT(ARGB1555, 90, 45));
            cmd(BITMAP_SIZE(NEAREST, BORDER, BORDER, 45, 45));
            
            cmd(BITMAP_HANDLE(2));
            cmd(BITMAP_SOURCE(RAM_G+20280+12348+4050));
            cmd(BITMAP_LAYOUT(ARGB1555, 108, 48));
            cmd(BITMAP_SIZE(NEAREST, BORDER, BORDER, 54, 48));
            
            cmd(BEGIN(BITMAPS));
            
            cmd(VERTEX2II(290, 170, 0, 0));//affiche Logo RADIOBOX
            
            /*Test botton Parametre*/
//            cmd(VERTEX2II(250, 170, 0, 0));//affiche Logo RADIOBOX      
//            cmd(VERTEX2II(411, 164, 2, 0));//affiche Logo parametre
            cmd(END());
            ...
            Fct_Bar(0,(Valeur_L*100)/819);//1V bar graphe au max ==> 4095/5 =819
            Fct_Bar(1,(Valeur_R*100)/819);//1V bar graphe au max ==> 4095/5 =819
...
//            int Config_UsartON = 0;
//            int Config_UsartOFF = 0;
//            static float ReducteurGain[2];
//            static bool ClickNext[2] = {0,0};
...
//                Valeur_Fader[i-2] = I2C_Read_Buffer(AD_FADERS_IN,Port_Song_IN[i-2]);
                ...
                if(Valeur_Fader[i-2] < Seuil_Fader+schmidt[i-2])
                {
                    ...
                    {
                        Muting_PT[i-1] = 1;
                        I2C_PT_Volume(Select_Input(i-6),PT_Muting);
                    }
                    if(OneX[i-6] == 1)//Pour envoyer une seul fois
                    {
                        Ok_Send = 1;
                        
//                        Table_Midi[0]=0x80;//envoie MIDI OFF
//                        Table_Midi[1]=0x39; 
//                        Table_Midi[2]=0x00; 
...
//                        appDataUart.currentState = APP_DRV_MSG_WRITE;
                ...
                else
                {
                    ...
                    OneX[i-6] = 1;
                    if(Affichage_BTN(i,ONE_CLICK) == 1)
                    {
                        Time_next[i-6] = 60;
                        Ok_Send = 1;//rearme le MIDI OFF

//                        Table_Midi[0]=0x90;//envoie MIDI ON
//                        Table_Midi[1]=0x39;
//                        Table_Midi[2]=0x3F;
                        Table_Midi[0]=Midi1;//envoie MIDI OFF
                        Table_Midi[1]=Midi2; 
                        Table_Midi[2]=Midi3; 
...
//                if((Volume_IN_Check[j] > Volume_IN_Change_Check[j]+32)&&(Start_Pot != true))
//                {
////                        Hyts_Pot[j] = +32; 
//                    Affiche_Phone[j] = j; 
//                    Cpt_Affiche = 40;
...
//                }
//                else if((Volume_IN_Check[j] < Volume_IN_Change_Check[j]-32)&&(Start_Pot != true))
//                {
////                        Hyts_Pot[j] = -32;
//                    Affiche_Phone[j] = j; 
//                    Cpt_Affiche = 40;
...
//                Volume_IN_Change[j]= Volume_IN[j];
//                Volume_IN_Change_Check[j]= Volume_IN_Check[j];
            }
...
////            Nconfig[Config_Used].Device_IN[0].ReductionMax
//            
//            if(Cpt_Affichage_Overload[0] > 0)
//            {
//                Cpt_Affichage_Overload[0]--;
//                cmd_text(230, 133, 23, 0,TxtAdj);
//            }
//            sprintf(TxtAdj,"Adj%2.2f",ReducteurGain[1]);
//            if(Cpt_Affichage_Overload[1] > 0)
//            {
//
//                Cpt_Affichage_Overload[1]--;
////                cmd_text(141, 133, 23, 0, "Overload");
//                cmd_text(230, 133, 23, 0,TxtAdj);
//            }
            /************Test*******************/
...
//                }
//                Cpt_Affichage_Overload[0] = Cpt_Affichage_Overload[0]+1;
//                if(Cpt_Affichage_Overload[0] > 40)
//                {
//                    int cli;
//                    cli++;
//                    if(cli <= 20)
...
//                }
//                Cpt_Affichage_Overload[1] = Cpt_Affichage_Overload[1]+1;
//                if(Cpt_Affichage_Overload[1] > 40)
//                {
//                    int cli;
//                    cli++;
//                    if(cli <= 20)
...
//                            cmd(COLOR_RGB(255,153,0)); 
//                        }
//                        sprintf(TxtAdj,"%2.2f Max",ReducteurGain[i]);   
//                    }
//                    else
//                    {
//                        cmd(COLOR_RGB(255,153,0));
//                        sprintf(TxtAdj,"%2.2f",ReducteurGain[i]);
//                    }          
//                    if(i == 0)
//                    {
//                      cmd_text(16, 133, 23, 0,TxtAdj);
...
//            }
            ...
            cmd_text(395, 240, 21, 0, "RDBX v1.0");
...
void Gestion_Line(void)
{
    ...
    for(i=0;i<2;i++)
    {
        ...
        if(Overload_Ligne[i] > NOverload)//0.2V
        {
            ...
            ReducteurGain[i] = ReducteurGain[i]+ 1.25;
            if(ReducteurGain[i] > Nconfig[Config_Used].Device_IN[i].ReductionMax)
            {
