...
#pragma config WDTPS =      PS4096//PS4096//PS16384
...
//    SYS_PORTS_Initialize();
    ...
    //sysObj.i2cDriver = DRV_I2C_Initialize(I2C_ID_2, &i2c_init_data);
    ...
    SYS_INT_VectorSubprioritySet(INT_VECTOR_T1, INT_SUBPRIORITY_LEVEL0);
     ...
     Soft_num = Read_EEPROM(1999);
     
    //Save_Adresse_IP[0] = Read_EEPROM(300); 
    for(i=300; i<368; i++)
    {
        if((i >= 300)&&(i <= 316))
        {
            Save_Adresse_IP[Cpt_Add] = Read_EEPROM(i);
            Cpt_Add++;
        }
        if((i >= 317)&&(i <= 333))
        {
            Save_Mask_IP[Cpt_Mask] = Read_EEPROM(i);
            Cpt_Mask++;
        }
        if((i >= 334)&&(i <= 350))
        {
            Save_Gateway_IP[Cpt_Gate] = Read_EEPROM(i);
            Cpt_Gate++;
        }
        if((i >= 351)&&(i < 367))
        {
            Save_DNS_IP[Cpt_DNS] = Read_EEPROM(i);
            Cpt_DNS++;
        }

    }
    if((Save_Adresse_IP[0] == -1)||(Save_Adresse_IP[0] == 0x0a)||(Save_Adresse_IP[0] == '0')||(Save_Adresse_IP[0] == 0))
    {
        sprintf(Save_Adresse_IP,"10.96.0.90");
        if(Soft_num == 22)
        {
            sprintf(Save_Adresse_IP,"10.96.0.91");
        }
        if(Soft_num == 23)
        {
            sprintf(Save_Adresse_IP,"10.96.0.98");
        }
        if(Soft_num == 24)
        {
            sprintf(Save_Adresse_IP,"10.96.0.95");
        }
        if(Soft_num == 25)
        {
            sprintf(Save_Adresse_IP,"10.96.0.96");
        }
        
        
        sprintf(Save_Mask_IP,"255.255.255.0");
        sprintf(Save_Gateway_IP,"10.96.0.1");
        sprintf(Save_DNS_IP,"10.96.0.1");
    }
     
    // Lecture_Word_EEPROM(&Save_Adresse_IP,300,316);
